...
export const description =
  "Discover available GitLab API operations from the OpenAPI spec. Call with no args for a tag overview, with tag to list operations in that tag, or with path for full parameter and request body details.";
...
export async function handler(input: { tag?: string; path?: string }) {
  ...
  if (tagMap.size === 1 && tagMap.has("untagged")) {
    ...
    return ResponseFormatter.success(
      `GitLab API — ${Object.keys(spec.paths).length} paths across ${segmentMap.size} API segments`,
      {
        baseUrl,
  ...
  return ResponseFormatter.success(
    `GitLab API — ${Object.keys(spec.paths).length} paths across ${tagMap.size} tags`,
    {
      baseUrl,
